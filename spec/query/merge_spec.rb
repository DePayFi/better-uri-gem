...
describe BetterUri::URI do
  ...
  context 'merge' do
    it 'adds get parameters' do
      expect(subject.query.merge(id: 1).to_s).to eq 'https://depay.fi?id=1'
    end

    context 'exi  sting query params' do
      subject { URI('https://depay.fi?id=1') }

      it 'changes existing parameters' do
        expect(subject.query.merge(id: 2).to_s).to eq 'https://depay.fi?id=2'
      end
    end

    context 'empty query params' do
      it 'does not change the original url' do
        expect(subject.query.merge({}).to_s).to eq 'https://depay.fi'
      end
    end
  end
  ...
  context 'merge!' do
    it 'changes the original object' do
      ...
      expect(subject.to_s).to eq 'https://depay.fi?id=1'
    end
    ...
    context 'existing query params' do
      ...
      it 'changes existing parameters' do
        ...
        expect(subject.to_s).to eq 'https://depay.fi?id=2'
      end
    end
